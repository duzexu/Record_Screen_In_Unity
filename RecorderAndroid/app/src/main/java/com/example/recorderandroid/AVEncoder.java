    ...
    */
    ...
    public void initAudioEncoder(int sampleRate, int pcmFormat,int chanelCount){
        ...
        if (audioCodecInfo == null) {
            if (DEBUG) Log.e(TAG, "=====Unable to find an appropriate codec for " + AUDIO_MIME_TYPE);
            return;
        }
    ...
    public void initVideoEncoder(int width, int height,int fps) {
        ...
        rotateBuffer = new byte[this.mWidth * this.mHeight * 3 / 2];
        Log.d(TAG, "====initVideoEncoder====width: "+mWidth+"  height: "+mHeight);
        ...
        if(mColorFormat == 0){
            Log.e(TAG,
                    "====couldn't find a good color format for " + vCodecInfo.getName()
                            + " / " + VIDEO_MIME_TYPE);
            return;
        }
        ...
        videoFormat.setInteger(MediaFormat.KEY_ROTATION, 180);
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("=====初始化视频编码器失败", e);
        }
        Log.d(TAG, String.format("=====编码器:%s创建完成", vEncoder.getName()));
    }
    ...
    private void selectColorFormat(MediaCodecInfo codecInfo,
        ...
        for (int i = 0; i < capabilities.colorFormats.length; i++) {
            ...
            Log.d(TAG,
                    "======selectColorFormat=====color format: " + colorFormat);
    ...
    private void startVideoEncode(){
        ...
        if (videoEncoderLoop) {
            throw new RuntimeException("====视频编码必须先停止===");
        }

        videoEncoderThread = new Thread() {
            @Override
            public void run() {
                Log.d(TAG, "===Video 编码线程 启动...");
                ...
                while (videoEncoderLoop && !Thread.interrupted()) {
                    try {
                        ...
                        if (DEBUG) Log.d(TAG, "======要编码的Video数据大小:" + data.length);
    ...
    private void startAudioEncode() {
        ...
        if (audioEncoderLoop) {
            throw new RuntimeException("====音频编码线程必须先停止===");
        }
        audioEncoderThread = new Thread() {
            @Override
            public void run() {
                Log.d(TAG, "====Audio 编码线程 启动...");
                presentationTimeUs = System.currentTimeMillis() * 1000;
                aEncoderEnd = false;
                ...
                while (audioEncoderLoop && !Thread.interrupted()) {
                    try {
                        ...
                        if (DEBUG) Log.d(TAG, "======要编码的Audio数据大小:" + data.length);
    ...
    public native static void Nv21ToI420(byte[] pNv21,byte[] pI420,int width,int height);
    public native static void Nv21ToYv12(byte[] pNv21,byte[] pYv12,int width,int height);
    public native static void Nv21ToNv12(byte[] pNv21,byte[] pNv12,int width,int height);
    public native static void Nv21ClockWiseRotate180(byte[] pNv21,int srcWidth,int srcHeight,byte[] outData,int[] outWidth,int[] outHeight);
    public native static void MirrorYuv(byte[] pNv21,int width,int height);
    ...
    private void encodeVideoData(byte[] input) {
        ...
        try {
            ...
            Log.d(TAG, "==1====Video===inputBufferIndex: " + inputBufferIndex+"  yuvLen: "+yuvBuffer.length);
            if (inputBufferIndex >= 0) { //输入缓冲区有效
                ByteBuffer inputBuffer = inputBuffers[inputBufferIndex];
                inputBuffer.clear();
                Log.d(TAG, "===2====Video===inputBufferIndex: " + inputBufferIndex+"  capacity: "+inputBuffer.capacity());
                //往输入缓冲区写入数据
                inputBuffer.put(yuvBuffer);
                Log.d(TAG, "===3=====Video===inputBufferIndex: " + inputBufferIndex+"  capacity: "+inputBuffer.capacity()+"  limit: "+inputBuffer.limit());
               ...
               // long pts = new Date().getTime() * 1000 - presentationTimeUs;
                ...
                if (vEncoderEnd) {
                    ...
                    vEncoder.queueInputBuffer(inputBufferIndex, 0, yuvBuffer.length,
...
//                    outputBuffer.get(outData);
    ...
    private void encodeAudioData(byte[] input){
        try {
            ...
            if (inputBufferIndex >= 0) { //输入缓冲区有效
                if (DEBUG) Log.d(TAG, "=======Audio===inputBufferIndex: " + inputBufferIndex);
                ByteBuffer inputBuffer = inputBuffers[inputBufferIndex];
                inputBuffer.clear();
                //往输入缓冲区写入数据
